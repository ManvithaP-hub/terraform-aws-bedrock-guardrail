...
locals {
  base_lambda_policy = [
    ...
    { Sid = "CW";    Effect = "Allow"; Action = ["cloudwatch:PutMetricData"]; Resource = "*";
      Condition = { StringEquals = { "cloudwatch:namespace" = local.metric_namespace } } },
  ]
}

# helper to create a Lambda role with an inline policy
...
resource "aws_iam_role_policy" "opa_evaluator" {
  ...
  policy = jsonencode({
    ...
    Statement = concat(local.base_lambda_policy, [
      # Live AWS state reads — no Config, no CloudTrail; direct SDK only
      {
        Sid = "EC2Read"
        Effect = "Allow"
...
resource "aws_iam_role_policy" "confidence_delta" {
  ...
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = concat(local.base_lambda_policy, [
      # Two Haiku invocations: one with guardrail, one without
...
resource "aws_iam_role_policy" "hitl_callback" {
  ...
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = concat(local.base_lambda_policy, [
      { Sid = "DDBUpdate"; Effect = "Allow"
        Action   = ["dynamodb:GetItem", "dynamodb:UpdateItem"]
        Resource = aws_dynamodb_table.hitl_approvals.arn },
      # Send task success / failure back to Step Functions
...
# ── Layer 4 – TTL expiry handler (DynamoDB Streams) ──────────────────────────
...
resource "aws_iam_role_policy" "ttl_expiry" {
  ...
  policy = jsonencode({
    ...
    Statement = concat(local.base_lambda_policy, [
      ...
      # Auto-rejection sends task failure to Step Functions
      { Sid = "SFNFailure"; Effect = "Allow"
...
resource "aws_iam_role_policy" "snapshot_creator" {
  ...
  policy = jsonencode({
    ...
    Statement = concat(local.base_lambda_policy, [
      ...
      # Read the current state of managed resources before snapshotting
      { Sid = "StateRead"; Effect = "Allow"
        Action   = ["ec2:DescribeInstances", "s3:ListAllMyBuckets", "iam:ListRoles", "cloudwatch:GetMetricData"]
        Resource = "*" },
    ])
  })
}
...
resource "aws_iam_role_policy" "rollback_executor" {
  ...
  policy = jsonencode({
    ...
    Statement = concat(local.base_lambda_policy, [
      ...
      # Rollback restores state via boto3; scope these to your actual resource set
      { Sid = "EC2Rollback"; Effect = "Allow"
        Action   = ["ec2:StartInstances", "ec2:StopInstances"]
        Resource = "*"
        Condition = { StringEquals = { "ec2:ResourceTag/ManagedByGuardrail" = "true" } } },
      { Sid = "SNSAlert"; Effect = "Allow"
        Action   = ["sns:Publish"]
        Resource = aws_sns_topic.hitl.arn },
    ])
  })
...
resource "aws_iam_role_policy" "sfn" {
  ...
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      { Sid = "InvokeLambdas"; Effect = "Allow"; Action = ["lambda:InvokeFunction"]
        Resource = [
      ...
      { Sid = "BedrockLayer1"; Effect = "Allow"; Action = ["bedrock:InvokeModel"]
        Resource = "arn:aws:bedrock:${local.region}::foundation-model/${var.confidence_delta_model_id}" },
      { Sid = "CWLogsDelivery"; Effect = "Allow"
